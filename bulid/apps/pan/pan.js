...
(function(G) {
	var pan = function() {
		...
		var lotterySuccess = function(m) {
			var html = '<div class="popup_l popLottery1" style="width:125px;"><img src="http://s1.ifiter.com/lottery/static/images/popup_img.jpg"></div>' + '<div class="popup_r" style="width:200px;">' + '<div class="popup_title">中奖了！</div>' + '<div class="popup_title1">您获得了<span>' + m + '</span>大米！</div>' + '<div class="share_title"> “分享到微博”可再得<span class="red">20大米</span></div>' + '<div class="share">' + '<a href="#"><span class="share_ico1 lottery"></span><span>新浪微博</span></a><a href="#"><span class="share_ico2 lottery"></span><span>搜狐微博</span></a>' + '<div class="clear"></div>' + '</div>' + '<div class="share">' + '<a href="#"><span class="share_ico3 lottery"></span><span>腾讯微博</span></a><a href="#"><span class="share_ico4 lottery"></span><span>网易微博</span></a>' + '<div class="clear"></div>' + '</div>' + '</div>' + '<div class="clear"></div>';
			return html;
		};
		...
		var updateroll = function(data, ret) {
			...
			for (var i = 0; i < data.length; i++) {
				html += '<li><a href="http://x.idongmi.com/u/' + data[i]['uid'] + '">' + data[i]['name'] + '</a>' + data[i]['msg'] + '</li>';
			}
			if (data.length === 0) {
					...
					$.ajax({
						...
						success: function(data) {
							...
							if (isLogin === 0 && ret.p != - 1) {
								//登录了
								html = laywrap(lotterySuccess(ret.p), false);
								GM.tools.overlay.fire(html);
							} else if (isLogin === 1 && ret.p !== - 1) {
								//没登陆     
								html = laywrap(lotterySuccess(ret.p), true);
								GM.tools.overlay.fire(html);
