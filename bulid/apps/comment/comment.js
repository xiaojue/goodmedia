   ...
   */
	...
	comment.prototype = {
		...
		drawComment: function() {
			var that = this,
			cg = that.config;
			var CommentTemp = '<div class="pcomment_sina pcmment_color">' + '<b class="b1"></b><b class="b2"></b><b class="b3"></b><b class="b4"></b>' + '<div class="pc_content">' + '<div class="pcomment_ico"><img src="http://s1.ifiter.com/static/images/comment/ico.gif"></div>' + '<div class="new_position">' + '<a href="javascript:void(0)" id="' + cg.faceid.slice(1) + '"><img src="http://s1.ifiter.com/static/images/comment/ico1.gif"></a>' + '<textarea class="lf" id="' + cg.textid.slice(1) + '"></textarea>' + '<a class="btn_normal" id="' + cg.subbtn.slice(1) + '" href="javascript:void(0);">评论</a>' + '<div class="clear"></div>' + '</div>' + '<div class="MIB_txtbl">' + '<span id="' + cg.countdownid.slice(1) + '"></span>' +
		 ...
		 */
		createCount: function(s) {
			var that = this,
			cg = that.config,
			count = '<div class="pcomment_text" id="' + cg.pco.slice(1) + '"><span class="blue">评论(<span id="' + cg.countid.slice(1) + '">' + s + '</span>)</span></div>';
		 ...
		 * @description 创建分页
		 */
		createPag: function(count, max, r) {
			var that = this,
			cg = that.config,
			...
			region = parseInt(cg.current, 10) + parseInt(r - 1, 10);
		 ...
		 * @description 渲染@人民函数
		 */
		drawingName: function(s) {
			var that = this,
			cg = that.config;
			var reg = /(@)(.*?)(\||\.|\,|\s|$)/g,
			str = s.replace(reg, function($0, $1, $2) {
				var txt = $2;
