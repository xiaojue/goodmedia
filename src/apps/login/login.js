				...
				$.ajax({
					...
					success: function(ret) {
						...
						try {
							...
							} else if (/register/gi.test(result)) {
								//var url = result.match(/register\.php\?action=.*\'/gi)[0].slice(0, - 1);
								//if ($.browser.msie) url += (url.indexOf('?') != - 1 ? '&': '?') + 'referer=' + escape(location.href);
								//location.href = 'http://bbs.idongmi.com/bbs/'+url;
                $.getScript('http://bbs.idongmi.com/java/bg_reg_bbs.php?uname='+username,function(){
                    if(GLOBAL_SYNC){
                      _fn.logintoBss(data);
                    }else{
                      alert('登录失败，请重试');
